...
var bv=new Vue({
	...
	methods:{
		...
		getdetailservicetype:function(){
			var _this=this;
			globalfunc.Cgetnot("login/serviseTypeSon", { id:1},function (res) {
				if(res.errno==0)
				{
		...
		addmappoint:function(distance,start,end){
			...
			// var icon1 = new qq.maps.MarkerImage('images/518/mai.png', size,origin,anchor);
			var marker1 = new qq.maps.Marker({
		        position: start,
		        map: this.smallmap
		    });
			// var icon2 = new qq.maps.MarkerImage('images/518/shou.png', size,origin,anchor);
		...
		confirmorder:function(){
			var send_name = this.sender;
			var send_mobile = this.sendtel;
			...
			if(send_name==""||send_name==null)
			{
				layer.msg("请输入发货人姓名");
				return false;
			}
			if(send_mobile==""||send_mobile==null)
			{
				layer.msg("请输入发货人电话");
				return false;
			}
			if(this.validatemobile(send_mobile,0)==false)
			{
				return false;
			}
			if(end_lon==null)
	...
	watch:{
		...
		inputgratuity:function(newv){
			...
			value=parseFloat(newv);
			this.inputgratuity=value;
		},
		totalmoney:function(newv){
			if(newv.toString()=="NaN")
			{
				this.totalmoney=parseFloat(this.runprice);
				this.inputgratuity=0;
				this.gratuity=0;
			}
		},
...
$(document).click(function(e){
	if(e.target.id!="receiveaddress"&&e.target.id!="sendaddress")
	{
