...
var bv=new Vue({
	...
	methods:{
		getbalance:function(){
		    ...
		    if (accesstoken) {
	    		var load=layer.load();
		      	globalfunc.Tpost("wallet/getBalance", "", accesstoken, function (res) {
		        	if (res.errno == 0) {
		        		_this.money=res.data.balance;
		        		layer.close(load);
		        	}else{
		        		layer.close(load);
		        		layer.msg("获取余额信息失败",{icon:5});
		        	}
		      	})
		    }
		},
		getdetaillist:function(){
		    ...
		    {
		    	var load=layer.load();
		    	globalfunc.Cpost("wallet/balanceLog?page="+this.nowpage, data, accesstoken, function (res) {
					if(res.errno==0)
					{
						layer.close(load);
						...
						for(var i=0;i<orderdata.length;i++)
						{
							...
							moneydetail.push(orderdata[i]);
						}
						_this.moneydetail=moneydetail;
					}else{
		        		layer.close(load);
		        		layer.msg("获取明细信息失败",{icon:5});
		        	}
				})
		    }
		},
...
pageNav.fn = function(p,pn){
    ...
    {
    	var load=layer.load();
    	globalfunc.Cpost("wallet/balanceLog?page="+p, data, accesstoken, function (res) {
			if(res.errno==0)
			{
				layer.close(load);
				totalpage=res.data.totalPages;
